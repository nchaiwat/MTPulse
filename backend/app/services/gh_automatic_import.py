...
from app.importers.gh import FILENAME_PATTERN, GhExtract, GhFormatError, extract_gh_file
...
logger = logging.getLogger("mtpulse.gh_automatic_import")


def process_gh_run(session: Session, run_id: int) -> None:
    run = session.get(ImportRun, run_id)
    if run is None or run.status != "running":
        return
    owner = session.get(ModernTrade, run.modern_trade_id)
    if owner is None or owner.code != "GH":
        raise AutomaticImportError("GH Run ต้องผูกกับ Global House coordinator")
    try:
        ...
        run.summary_message = f"พบ {len(candidates)} ไฟล์ · กำลังตรวจสอบ GH"
        session.commit()
        results = []
        ...
        logger.exception("GH automatic import run %s failed", run_id)
        session.rollback()
        run = session.get(ImportRun, run_id)
        assert run is not None
        run.status = "failed"
        run.finished_at = bangkok_now()
        run.error_message = f"ประมวลผล GH ไม่สำเร็จ: {type(exc).__name__}: {exc}"[:1000]
        run.summary_message = "GH Run ไม่สำเร็จและไม่มีการลบข้อมูลเดิม"
        session.commit()
        send_telegram(
            session,
            "❌ Automatic Import Global House (GH) ไม่สำเร็จ",
...
def _process_candidate(
    ...
    *,
    ...
    try:
        ...
        batch = import_gh_file(session, extract, actor=run.requested_by)
        row = session.get(SourceFile, row.id)
        assert row is not None
        return _save_outcome(
            session, row, candidate, extract, "imported", "นำเข้า Global House (GH) สำเร็จ", batch.id
        )
...
def _download(candidate: SourceCandidate, *, username: str, password: str) -> GhExtract:
    with tempfile.TemporaryDirectory(prefix="mtpulse-gh-") as temp_dir:
