...
from app.importers.ta import TaExtract
...
class TaImportError(ValueError):
    pass


def ensure_ta_trade(session: Session) -> ModernTrade:
    ...
    if modern_trade is None:
        ...
        modern_trade.name = "Thai-Aust"
        modern_trade.vat_mode = "include"
        modern_trade.show_unmatched_items = False
        modern_trade.show_unmatched_branches = False
        modern_trade.source_group_code = "TA"
        modern_trade.source_subfolder = modern_trade.source_subfolder or "TA"
        modern_trade.branch_prefix = "GH"
    return modern_trade
...
def import_ta_file(
    ...
    *,
    ...
    modern_trade = ensure_ta_trade(session)
    ...
    if existing and existing.business_fingerprint == extract.business_fingerprint:
        raise TaImportError("ข้อมูลธุรกิจชุดนี้ถูกนำเข้าแล้ว แม้ชื่อไฟล์อาจต่างกัน")
    if existing:
        session.execute(
...
def _checksum(fingerprint: str) -> str:
    return sha256(f"{fingerprint}:TA".encode()).hexdigest()
