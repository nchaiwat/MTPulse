...
def test_pairing_selects_latest_generation_and_marks_older_superseded() -> None:
    ...
    old_inventory = candidate(
        "VRM_0000003970_InventoryData_20260906_20260907_070100.zip"
    )
    new_inventory = candidate(
        "VRM_0000003970_InventoryData_20260906_20260907_070129.zip"
    )
    sales = candidate(
        "VRM_0000003970_SalesData_20260906_20260907_070052.zip"
    )
...
def test_corrected_pair_replaces_both_mts_and_writes_one_audit(tmp_path, monkeypatch) -> None:
    ...
    with Session(engine) as session:
        ...
        session.commit()
        first_ids = {
            batch.modern_trade_id: batch.id
        ...
        audit = session.scalar(
            select(AuditEvent).where(AuditEvent.action == "corrected_reimport")
        )
        corrected_ids = {batch.id for batch in batches.values()}
        corrected_fingerprints = {
            batch.business_fingerprint for batch in batches.values()
        }
