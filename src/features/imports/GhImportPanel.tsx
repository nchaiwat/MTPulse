...
export function GhImportPanel({ onCompleted }: { onCompleted: () => void }) {
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<GhImportPreview | null>(null);
  ...
  const inspect = async () => {
    ...
    setPreview(null);
    try {
      setPreview(await previewGhImport(file, setProgress));
    } catch (error) {
      setMessage(
        error instanceof Error
          ? error.message
          : "ตรวจสอบไฟล์ Global House ไม่สำเร็จ",
      );
      onCompleted();
    } finally {
      setProgress(null);
  ...
  const confirm = async () => {
    ...
    setMessage(null);
    try {
      const result = await confirmGhImport(
        file,
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "นำเข้าข้อมูล Global House ไม่สำเร็จ",
      );
      onCompleted();
    } finally {
      setProgress(null);
  ...
  return (
    <section
      ...
      role="tabpanel"
      aria-labelledby="import-tab-GH"
    >
      <header className="import-workflow-heading">
        <div>
          <span className="eyebrow">GH manual import</span>
          <h2>Global House (GH)</h2>
        </div>
        <span className="import-format-note">Daily · Piyawat .xlsx</span>
      </header>
      <div className="hp-mh-source-note">
        <Building2 size={18} aria-hidden="true" />
        <p>
          ระบบอ่านวันที่จากชื่อไฟล์ Piyawat แล้วใช้เป็นวันที่รับไฟล์
      ...
      <div className="hp-mh-file-grid">
        <label>
          <span>1 · Piyawat-YYYY-MM-DD*.xlsx</span>
          <input
            type="file"
            accept=".xlsx"
            disabled={busy !== null}
          ...
          <header>
            <div>
              ...
              <div>
                ...
                <h3>Global House · {formatDisplayDate(preview.dataDate)}</h3>
              </div>
            </div>
            <span
          ...
          <footer>
            ...
            >
              {busy === "confirm"
                ? "กำลังบันทึก…"
                : preview.operation === "replace"
                  ? "ยืนยันแทนที่ข้อมูล Global House"
                  : "ยืนยันนำเข้า Global House"}
            </button>
          </footer>
        </div>
      )}
