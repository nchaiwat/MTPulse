...
function ImportProgress({ progress }: { progress: WorkProgress }) {
  ...
  const label = progress.phase === 'fileshare' ? 'กำลังอ่านไฟล์จาก FileShare' : progress.phase === 'importing' ? 'กำลังตรวจสอบและนำเข้าข้อมูล' : uploading ? 'กำลังส่งไฟล์เข้า Server' : 'กำลังอ่าน Excel และตรวจข้อมูล'
  return (
    <div className="import-transfer" role="status">
      <div>
        <span>{label}</span>
        <strong>{uploading ? `${progress.percent}%` : 'โปรดรอสักครู่'}</strong>
      </div>
      <div className="import-transfer-track" data-indeterminate={!uploading || undefined} role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={100} aria-valuenow={uploading ? progress.percent : undefined}>
        <span style={{ width: uploading ? `${progress.percent}%` : '38%' }} />
      </div>
      {uploading && progress.total > 0 && (
        <small>
          {number.format(progress.loaded / 1024)} / {number.format(progress.total / 1024)} KB
        </small>
      )}
    </div>
  )
...
export function ImportPage({ correctiveBatchId = null }: { correctiveBatchId?: number | null }) {
  ...
  const visibleActivities = activeTab === 'overview' ? activities : activities.filter((item) => item.mtCode.toUpperCase() === activeTab)
  ...
  const inspectHpMh = async () => {
    ...
    setHpMhPreview(null)
    try {
      setHpMhPreview(await previewHpMhImport(hpMhFiles.inventory, hpMhFiles.sales, setProgress))
    } catch (error) {
      setHpMhMessage(error instanceof Error ? error.message : 'ตรวจสอบคู่ไฟล์ HP/MH ไม่สำเร็จ')
      void loadActivity()
    } finally {
      setProgress(null)
      setBusy(null)
  ...
  const confirmHpMh = async () => {
    if (!hpMhPreview?.canImport || !hpMhFiles.inventory || !hpMhFiles.sales) return
    setBusy('confirm')
    setHpMhMessage(null)
    try {
      const result = await confirmHpMhImport(hpMhFiles.inventory, hpMhFiles.sales, hpMhPreview.businessFingerprint, setProgress)
      setHpMhMessage(`${result.message} · ${result.notification.message}`)
      setHpMhFiles({ inventory: null, sales: null })
      setHpMhPreview(null)
      void loadActivity()
    } catch (error) {
      setHpMhMessage(error instanceof Error ? error.message : 'นำเข้าข้อมูล HP/MH ไม่สำเร็จ')
      void loadActivity()
    } finally {
      setProgress(null)
      setBusy(null)
    ...
    <div className="import-page page-content">
      ...
      <nav className="import-workspace-tabs" role="tablist" aria-label="Modern Trade">
        {workspaceTabs.map((tab) => {
          const activityCount = tab.code === 'overview' ? activities.length : activities.filter((item) => item.mtCode.toUpperCase() === tab.code).length
            ...
            <button
              ...
              onKeyDown={(event) => {
                ...
                if (event.key === 'ArrowRight' || event.key === 'ArrowLeft') {
                  ...
                  const nextIndex = (currentIndex + step + workspaceTabs.length) % workspaceTabs.length
                  moveWorkspaceFocus(workspaceTabs[nextIndex].code)
                }
                if (event.key === 'Home' || event.key === 'End') {
                  event.preventDefault()
                  moveWorkspaceFocus(event.key === 'Home' ? workspaceTabs[0].code : workspaceTabs[workspaceTabs.length - 1].code)
                }
              }}
            >
              {tab.code === 'overview' ? <LayoutList size={16} aria-hidden="true" /> : <Building2 size={16} aria-hidden="true" />}
              <span>
      ...
      {activeTab === 'TWD' && (
        <section id="import-panel-TWD" className="import-workflow" role="tabpanel" aria-labelledby="import-tab-TWD">
            ...
            <>
              <div className="import-source-bar">
                ...
                <ol className="import-pipeline" aria-label="ขั้นตอนนำเข้าข้อมูล">
                  <li data-active={!preview || undefined}>
                    <span>1</span>
                    {sourceMode === 'upload' ? 'ส่งไฟล์' : 'อ่านไฟล์'}
                  </li>
                  <li data-active={preview ? true : undefined}>
                    <span>2</span>ตรวจสอบ
                  </li>
                  <li>
                    <span>3</span>ยืนยัน
                ...
                <section className="fileshare-ready-stage" aria-labelledby="fileshare-ready-heading">
                  <header>
                    <div>
                      <span className="stage-number">1</span>
                      <div>
                        <span className="eyebrow">File registry</span>
                        <h3 id="fileshare-ready-heading">ไฟล์พร้อมนำเข้า</h3>
                      </div>
                    </div>
                    <button type="button" className="secondary-action" disabled={readyState === 'loading' || busy !== null} onClick={() => void loadReadyFiles()}>
                      <RefreshCw size={14} className={readyState === 'loading' ? 'is-spinning' : undefined} aria-hidden="true" />
                      Refresh
                    </button>
                  </header>
                  {readyState === 'loading' && (
                    <div className="fileshare-ready-state" role="status">
                      <LoaderCircle className="is-spinning" size={18} />
                      กำลังอ่านทะเบียนไฟล์…
                    </div>
                  )}
                  {readyState === 'error' && (
                    <div className="fileshare-ready-state error" role="alert">
                      <AlertTriangle size={18} />
                      โหลดรายการไม่สำเร็จ{' '}
                      <button type="button" onClick={() => void loadReadyFiles()}>
                        ลองใหม่
                      </button>
                    </div>
                  )}
                        ...
                        <tbody>
                          {readyFiles.map((readyFile) => (
                            <tr key={readyFile.id} data-selected={selectedSourceFileId === readyFile.id || undefined}>
                              ...
                              <td>
                                <strong>{readyFile.filename}</strong>
                              </td>
                              <td>{number.format(readyFile.sizeBytes / 1024)} KB</td>
                              <td>{formatDisplayDateTime(readyFile.discoveredAt)}</td>
                              <td>
                                <button type="button" disabled={busy !== null} onClick={() => void inspectFileShare(readyFile.id)}>
                                  ตรวจสอบ
                                </button>
                              </td>
                            </tr>
              ...
              {preview && (
                <div className="preview-stage" data-blocked={!preview.canImport || undefined}>
                  <header>
                    <div>
                      ...
                      <div>
                        ...
                        <h3>
                          {preview.detectedMtName} · {formatDisplayDate(preview.dataDate)}
                        </h3>
                      </div>
                    </div>
                  ...
                  <dl className="preview-grid">
                    <div>
                      <dt>MT</dt>
                      <dd>{preview.detectedMt}</dd>
                    </div>
                    <div>
                      <dt>วันที่ข้อมูล</dt>
                      <dd>{formatDisplayDate(preview.dataDate)}</dd>
                    </div>
                    <div>
                      <dt>รายการ</dt>
                      <dd>{number.format(preview.rowCount)}</dd>
                    </div>
                    <div>
                      <dt>SKU</dt>
                      <dd>{number.format(preview.skuCount)}</dd>
                    </div>
                    <div>
                      <dt>Branch</dt>
                      <dd>{number.format(preview.branchCount)}</dd>
                    </div>
                    <div>
                      <dt>Amount</dt>
                      <dd>{number.format(preview.amount)}</dd>
                    </div>
                    <div>
                      <dt>Sales Qty</dt>
                      <dd>{number.format(preview.salesQty)}</dd>
                    </div>
                    <div>
                      <dt>Return rows</dt>
                      <dd>{number.format(preview.negativeRowCount)}</dd>
                    </div>
                  </dl>
      ...
      {activeTab === 'overview' && (
        <section id="import-panel-overview" className="import-overview-panel" role="tabpanel" aria-labelledby="import-tab-overview">
          <header>
            <span className="eyebrow">Import overview</span>
            <h2>สถานะล่าสุดแยกตาม Modern Trade</h2>
          </header>
          <div className="import-mt-ledger">
            {workspaceTabs.slice(1).map((tab) => {
              const mtActivities = activities.filter((item) => item.mtCode.toUpperCase() === tab.code)
              ...
              return (
                <article key={tab.code}>
                  ...
                  <div>
                    ...
                    <small>{latest ? latest.message : 'ยังไม่มีประวัติการนำเข้า'}</small>
                  </div>
                  <div>
                    <b>{mtActivities.length}</b>
      ...
      {(activeTab === 'HP' || activeTab === 'MH') && (
        <section id={'import-panel-' + activeTab} className="import-workflow import-hp-mh-workflow" role="tabpanel" aria-labelledby={'import-tab-' + activeTab}>
          <header className="import-workflow-heading">
            <div>
              <span className="eyebrow">HP/MH manual import</span>
              <h2>
                {selectedTab.name} ({activeTab})
              </h2>
            </div>
          ...
          {manualMode === 'folder' ? (
            ...
            <>
              <div className="hp-mh-source-note">
                ...
                <p>HP และ MH ใช้คู่ไฟล์เดียวกัน ระบบจะตรวจ Inventory/Sales และนำเข้าทั้งสอง MT พร้อมกัน โดยแยกข้อมูลของแต่ละ MT ตาม Logic เดิม</p>
              </div>
              ...
              <div className="hp-mh-file-grid">
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      const selectedFile = event.target.files?.[0] ?? null
                      setHpMhFiles((current) => ({
                        ...current,
                        inventory: selectedFile,
                      }))
                      setHpMhPreview(null)
                      setHpMhMessage(null)
                    }}
                  />
                  <small>{hpMhFiles.inventory ? `${hpMhFiles.inventory.name} · ${number.format(hpMhFiles.inventory.size / 1024)} KB` : 'ZIP ที่ภายในมี InventoryData.csv'}</small>
                </label>
                <label>
                  ...
                  />
                  <small>{hpMhFiles.sales ? `${hpMhFiles.sales.name} · ${number.format(hpMhFiles.sales.size / 1024)} KB` : 'ZIP ที่ภายในมี SalesData.csv'}</small>
                </label>
                <button className="primary-action hp-mh-preview-action" type="button" disabled={!hpMhFiles.inventory || !hpMhFiles.sales || busy !== null} onClick={() => void inspectHpMh()}>
              ...
              {hpMhPreview && (
                <div className="preview-stage hp-mh-preview" data-blocked={!hpMhPreview.canImport || undefined}>
                      ...
                      return (
                        <article key={code}>
                          ...
                          <dl>
                            <div>
                              <dt>รายการ</dt>
                              <dd>{number.format(summary.rowCount)}</dd>
                            </div>
                            <div>
                              <dt>SKU</dt>
                              <dd>{number.format(summary.skuCount)}</dd>
                            </div>
                            <div>
                              <dt>Branch</dt>
                              <dd>{number.format(summary.branchCount)}</dd>
                            </div>
                            <div>
                              <dt>Amount</dt>
                              <dd>{number.format(summary.amount)}</dd>
                            </div>
                            <div>
                              <dt>Sales Qty</dt>
                              <dd>{number.format(summary.salesQty)}</dd>
                            </div>
                            <div>
                              <dt>Stock</dt>
                              <dd>{number.format(summary.stockOnHand)}</dd>
                            </div>
                          </dl>
                        </article>
                      )
                  ...
                  <footer>
                    ...
                    <button className="primary-action" type="button" disabled={!hpMhPreview.canImport || busy !== null} onClick={() => void confirmHpMh()}>
      ...
      {activeTab === 'GH' && (
        <>
          <GhImportPanel onCompleted={() => void loadActivity()} />
          <FolderImportPanel expectedSourceGroup="GH" onCompleted={() => void loadActivity()} />
        </>
      )}

      {activeTab === 'TA' && (
        <>
          <TaImportPanel onCompleted={() => void loadActivity()} />
          <FolderImportPanel expectedSourceGroup="TA" onCompleted={() => void loadActivity()} />
        </>
      )}
        ...
        <header>
          <div>
            ...
            <div>
              ...
              <h3 id="activity-heading">ประวัติการทำงาน · {selectedTab.label}</h3>
            </div>
          </div>
          ...
          {visibleActivities.map((item) => (
            <article key={item.id}>
              ...
              <div>
                ...
                <small>
                  {item.mtCode} · {item.filename}
                  {item.dataDate ? ` · ข้อมูล ${formatDisplayDate(item.dataDate)}` : ''}
                </small>
              </div>
              <div className="activity-time">
                <span>{statusLabel(item.status)}</span>
                <time>{formatDisplayDateTime(item.occurredAt)}</time>
              </div>
            </article>
          ))}
        </div>
